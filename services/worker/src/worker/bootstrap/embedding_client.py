"""LLM-proxy embedding client — PR #19a Group A.

Thin ``httpx.AsyncClient`` wrapper over
``POST {base_url}/api/v1/embedding`` on the local llm-proxy service
(see ``services/llm-proxy``, PR #18). Two callers share this module:

1. ``worker.bootstrap.upsert.upsert_report`` — embed on ingest.
2. ``worker.bootstrap.cli`` ``backfill-embeddings`` subcommand —
   one-shot population of existing ``reports`` rows where
   ``embedding IS NULL``.
