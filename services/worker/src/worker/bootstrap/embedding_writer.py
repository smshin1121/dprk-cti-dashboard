"""Embed-on-ingest orchestrator for worker bootstrap — PR #19a Group B.
...
2. **Call llm-proxy** via the injected
   :class:`~worker.bootstrap.embedding_client.LlmProxyEmbeddingClient`.
   Transient errors (429 / 5xx / timeout) are caught here, logged
   WARN, and turned into a ``SKIPPED_TRANSIENT`` return. Permanent
   errors (422 / dimension mismatch / malformed 2xx) propagate for
   the caller to log at ERROR and swallow — enrichment never blocks
   the ingest/promote gate (plan C4).
...
3. **PostgreSQL-only UPDATE** of ``reports.embedding`` guarded by
   ...
   the ``embedding`` column (see ``tables.py`` module docstring).

``upsert_report`` stays pgvector-free; this module is the only
place in the worker codebase that references the pgvector column.
"""
