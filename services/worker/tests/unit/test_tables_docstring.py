...
Known limitation (acceptable trade-off): the ``\\b\\d{4}\\b`` regex
matches ANY 4-digit number in the docstring, not only migration
prefixes. A future docstring that mentions an unrelated 4-digit value
(a year like ``2026``, a port number, etc.) could mask a forgotten
migration if that unrelated number is >= the actual migration max.
Current docstrings contain no such matches; contributors should keep
migration mentions canonical (``NNNN`` form) and avoid bare 4-digit
decoration when editing. A future test could tighten this with a
``NNNN_`` underscore suffix or a fenced "Migrations covered:"
section, but the leniency is preferred today.
"""
