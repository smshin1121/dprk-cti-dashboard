"""Regression test: ``services/worker/Dockerfile`` pins every base image to ``@sha256:<digest>``.
...
def _service_dockerfile() -> Path:
    """Locate ``services/worker/Dockerfile`` from this test file.

    Layout: ``<repo>/services/worker/tests/unit/test_dockerfile_digest_pin.py``
    so ``parents[2]`` is ``services/worker/``.
    """
...
def test_every_from_is_digest_pinned() -> None:
    ...
    assert not unpinned, (
        f"services/worker/Dockerfile has FROM directives without an "
