"""Regression test: ``services/llm-proxy/Dockerfile`` pins every base image to ``@sha256:<digest>``.
...
def _service_dockerfile() -> Path:
    """Locate ``services/llm-proxy/Dockerfile`` from this test file.

    Layout: ``<repo>/services/llm-proxy/tests/unit/test_dockerfile_digest_pin.py``
    so ``parents[2]`` is ``services/llm-proxy/``.
    """
...
def test_every_from_is_digest_pinned() -> None:
    ...
    assert not unpinned, (
        f"services/llm-proxy/Dockerfile has FROM directives without an "
