...
5. **No FE wiring in this PR.** OpenAPI snapshot lands here; FE component code, hooks, route registration, i18n keys for the UI banner — all in PR B (`pr{N}-correlation-fe.md`).

---

## 0.1 Implementation amendment (post-lock, 2026-05-03)

The locked plan above placed the Pact contract (5 interactions per umbrella §7.6) in PR A scope. **During implementation that scope was deferred to PR B.** This deviation is recorded for audit; the umbrella spec §7.6 contract requirement is unchanged — only the PR carrying it shifts.

**Why deferred:**
1. Pact in this repo is consumer-driven: `contracts/pacts/frontend-dprk-cti-api.json` is generated by FE consumer tests and the BE provider verification runs against the committed file. Hand-authoring 5 interactions in PR A would diverge from the shape PR B's FE consumer tests will actually emit.
2. `pattern_pact_dependency_override_via_provider_state` requires state handlers wired to consumer needs that are not yet authored. PR B authors them alongside the FE pact-generation tests.
3. The pre-existing `PACT_PROVIDER_BASE_URL` CI failure (unchanged from `main`) means provider verification CI gating is currently inactive — adding interactions in PR A without a consumer to drive them adds maintenance surface for zero gating value.

**Consequence for this PR:**
- `services/api/tests/contract/test_pact_correlation_provider.py` (planned in §3.1, §9.3) — NOT shipped.
- `contracts/pacts/frontend-api-correlation.json` — NOT created (existing `frontend-dprk-cti-api.json` is unchanged).
- pact_states.py extension for correlation states — NOT shipped.

**Consequence for PR B:** Pact contract for `/correlation` and `/correlation/series` MUST land in PR B alongside FE consumer wiring; umbrella §7.6 5-interaction list applies there.
