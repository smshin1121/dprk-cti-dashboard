...
- **2026-05-08 (§0.1 amendment — T8 Pact contract LoC + 49-cell length-pin)** — Plan §4 T8 row + §B7 said `+~250 LoC at most`. Actual T8 base + r1 fold delivered substantially more — base commit `8349a94` was `+2022 LoC` (test +614 / pact JSON +1409); the r1 fold replaces every `eachLike(...)` / `arrayContaining(...)` over `lag_grid` with explicit 49-cell raw-array literals so pact-ruby length-pins to exactly 49 positionally (Codex r1 CRITICAL #1: `eachLike` generates `min: 1`; `arrayContaining` doesn't pin total length; both miss the umbrella §4.4 + BE pydantic + FE Zod 49-cell lock at provider-verify time). The locked 6-field × 2-method × 49-cell shape × 3 happy interactions × explicit per-cell matchers makes `~250 LoC` unrealistic. The locked 49-cell length-pin is non-negotiable per the umbrella spec; the LoC growth is the cost of getting the contract right. The §B7 LoC estimate is hereby treated as descriptive (per `pattern_plan_section_precedence_4_normative_5_descriptive` — §4 task-table normative, §B7 mid-prose descriptive). The base commit's `25 → 30` Pact-interaction count was also miscounted; actual delta is `+5` (`21 → 26`) because `grep -c '"description"'` overcounts by the 4 extra `description` strings in pact metadata; the actual interactions count is `j.interactions.length`. T8 r1 fold corrects both via this amendment + the matcher-rework commit. Per `pattern_plan_vs_impl_section_0_1_amendments`.

- **2026-05-09 (§0.1 amendment — T9 §8 Q4 alpha exposure default vs §B3 + §7.5 cache-key isomorphism)** — Surfaced by Codex T12 r1 against the PR body draft. §8 Q4 default at line 173 said "FE always sends without `alpha` and the BE applies its 0.05 default." This contradicted §B3 hook signature (`useCorrelation(x, y, dateFrom, dateTo, alpha)` — 5 positional args including alpha) AND §7.5 cache-key isomorphism (umbrella line 576: BE Redis cache key is `correlation:v1:{x}:{y}:{date_from}:{date_to}:{alpha}` — alpha is in the BE key). T9 implementation supplies the literal `0.05` from `const ALPHA = 0.05` in `CorrelationPage.tsx:60`, passes it through `useCorrelation(...)` (`CorrelationPage.tsx:151-156`), through `analyticsCorrelation(...)` query key (`queryKeys.ts`), through `getCorrelation(...)` helper (`endpoints.ts:471, :474`), and `toCorrelationQueryParams(...)` (`analyticsFilters.ts:185, :192`) writes `alpha=0.05` into the URL query string. The endpoint helper docstring at `endpoints.ts:455` ("`alpha` is always supplied") and `analyticsFilters.ts:175-176` ("`alpha` is always emitted — the BE Redis cache key includes it") already document the rationale in code; only §8 Q4 prose has the stale "without alpha" wording. Effective §8 Q4 default reads as: "FE does not surface alpha in the filter UI; the hook always supplies the literal 0.05 to maintain cache-key isomorphism with BE Redis (umbrella §7.5)." No code change — the implementation has been correct since T5; only the §8 default wording is corrected here (in the change log; §8 prose itself is preserved verbatim as the original audit trail). Per `pattern_plan_vs_impl_section_0_1_amendments`.

- **2026-05-09 (§0.1 amendment — T9 §8 Q1 catalog dropdown grouping deferred)** — Surfaced by Codex T12 r1 against the PR body draft. §8 Q1 default at line 170 said "single flat dropdown grouped by root via section headers." T9 implementation in `CorrelationFilters.tsx:94` ships flat ungrouped (`{catalog.map((s) => ...)}` with no `[ Reports ]` / `[ Incidents ]` group-headers). Reason for the deviation: section grouping is purely cosmetic — it does NOT affect URL state (B5: `x` and `y` carry the full opaque series ID, root is recoverable from the catalog response), the React Query cache key (B3: catalog is a single flat array), the test contract (B8 (b) URL-state tests pin per-option testids `correlation-filter-y-option-<id>`, which work the same with or without group headers), or the BE surface (no catalog mutation). Implementing grouping inside T9 would have required (a) restructuring the disclosure-button list into a per-root section + (b) authoring i18n keys for the two section labels in both ko and en + (c) extending the test contract with section-presence assertions — net ~80 LoC of cosmetic UX without functional value, which would have inflated T9's 1.5d budget. Grouping is layered in additively in PR-C hardening (umbrella §11) or a small follow-up PR; current PR ships flat ungrouped per the deviation. No B-row policy invariant changed; no umbrella requirement violated. Per `pattern_plan_vs_impl_section_0_1_amendments`.
