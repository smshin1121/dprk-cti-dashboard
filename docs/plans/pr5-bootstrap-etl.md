# PR #5 Plan — Bootstrap ETL + Alias Dictionary
...
**Successors:** PR #6 (Great Expectations + audit row-level), PR #7 (RSS/TAXII worker + Prefect)

---

## 1. Goal

Load the v1.0 workbook (Actors / Reports / Incidents — ~2,372 rows total) into the existing Phase 0 schema as **canonical, deduplicated, idempotent data** so downstream API read paths, data-quality checks (PR #6), and analytics views (Phase 3+) have a real corpus to operate on.
...
| **D2** | Great Expectations and audit-log row-level expansion are **out of scope** for PR #5. They land in PR #6. | Splits rollback units. PR #5 is already wide (input parsing, normalization, upsert, CLI). GE adds a full new dependency + fixtures footprint that should not block ETL landing. |
| **D3** | CLI-first: `python -m worker.bootstrap --workbook <path> [--dry-run] [--limit N]`. No Prefect flow in PR #5. | Bootstrap is one-shot. Prefect orchestration is only justified when RSS/TAXII scheduling arrives in PR #7. |
| **D4** | BigInt PK migration is a **separate preflight PR** (PR #4, branch `feat/p1.x-bigint-pk`) landing before PR #5 implementation starts. | Tables are empty right now — ALTER is cheap. Bundling with ETL makes the rollback unit too large. |
...
- Great Expectations suite → PR #6
- Audit-log row-level insert for bootstrap → PR #6
- Prefect flow registration → PR #7
- RSS/TAXII worker → PR #7
- BigInt PK migration → PR #4 (preflight)
...
Dependency order. T1 lands in the preflight PR (#4); T2–T9 land in PR #5.

| # | Task | Depends on | Est. | Exit criteria |
|:---:|:---|:---|:---:|:---|
| **T1** | BigInt PK migration (preflight PR #4). `0004_bigint_pk_migration.py` — ALTER all Integer PK/FK columns (`groups`, `codenames`, `sources`, `reports`, `tags`, `report_tags`, `incidents`, `techniques`, `malware`, `vulnerabilities`, + staging FKs). | PR #3 merged | 0.5d | `alembic upgrade head` + `downgrade -1` + `upgrade head` green in CI; no Integer PKs remain in schema dump |
...
**Total estimate:** ~5 dev-days for PR #5, plus ~0.5d for PR #4 preflight.
...
| BigInt migration (T1) breaks existing `services/api` SQLAlchemy models | Med | PR #4 must also update `services/api` model column types where present; CI `api-tests` job is the gate. |

---

## 6. Rollback Plan

- **PR #4 (BigInt)**: Alembic `downgrade -1` reverses. No data loss because tables are empty today. Verified by CI `db-migrations` job.
- **PR #5**: Pure additive (new package under `services/worker`, new YAML under `data/`, new CI job). Revert = `git revert`. Running `worker.bootstrap` against prod DB is manual and gated by operator, so revert leaves the DB untouched.
...
1. Preflight PR #4 is merged to main.
...
- **Workbook delivery path**: the real v1.0 workbook is not yet staged on this dev box. Blocker for local end-to-end verification against real data before merge. Mitigation: PR #5 CI uses the fixture; real-data smoke happens locally post-merge, with a follow-up issue if column drift is detected.
