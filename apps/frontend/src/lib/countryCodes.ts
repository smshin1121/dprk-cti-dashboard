 ...
 *
 * The bundled Natural Earth 110m TopoJSON tags each country feature
 * with ``id`` = ISO 3166-1 **numeric** (string form, e.g. "408" for
 * DPRK). The BE `/analytics/geo` response uses ISO 3166-1 **alpha-2**
 * (e.g. "KP"). This table bridges the two.
 *
 * Scope:
 *   Full ISO 3166-1 numeric → alpha-2 mapping (~250 entries). The
 *   map is static data — reviewers can grep for any code in isolation.
 *   Returned `undefined` on unknown input lets the map layer render
 *   a country with a neutral "no-data" fill rather than crashing.
 *
 * Plan D7 reminder:
 *   DPRK (numeric 408, alpha-2 KP) is highlighted by its geographic
 *   identity on the map, not by data-payload special-case. The BE
 *   returns KP as a plain country row; this table resolves KP for
 *   every country, then the FE visually layers the highlight on the
 *   KP path.
 */
...
/**
 * Normalize TopoJSON feature id (either a string "408" or a number
 * 408 depending on the file) to a zero-padded 3-char string for map
 * lookup. Returns undefined for inputs that can't be coerced.
 */
function normalizeNumericId(id: unknown): string | undefined {
...
export function numericToIso2(id: unknown): string | undefined {
  const key = normalizeNumericId(id)
  if (key == null) return undefined
  return NUMERIC_TO_ISO2[key]
}

/** DPRK's ISO 3166-1 numeric — used by the map's feature-identity
 *  highlight layer. KP's count can still be zero or absent in the
 *  BE response (plan D7: DPRK has no special-case field); the
 *  highlight is geographic, not data-driven. */
export const DPRK_NUMERIC_ID = '408'
export const DPRK_ISO2 = 'KP'
